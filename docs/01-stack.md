...
| Accès données | **Prisma** | Migrations versionnées (indispensable pour un référentiel qui évolue), typage généré. *Compromis assumé : Drizzle serait plus proche du SQL et plus léger ; Prisma est préféré pour la robustesse des migrations en production.* |
...
- aucun accès réseau, base ou fichier (uniquement des *ports* : interfaces) ;
